...
export function buildScriptSystemPrompt(
  ...
  if (preset === 'shot-compose') {
    ...
    return [
      ...
      '2) 从清单里挑出这一镜**画面里确实出现**的素材（只能从清单原样选取，名称必须与清单完全一致；本镜没用到就给空数组）。',
      '**严格精确、宁缺勿滥**：角色——仅当该角色在这一镜出镜时才选；场景——仅当镜头发生在该场景时才选（不同镜头往往在不同场景，别把所有场景都选上）；道具——仅当该道具在画面中出现时才选。一个镜头通常只用到 1 个场景、0~2 个角色、0~1 个道具。绝不要因为「整部片子有这个素材」就把它选进每一镜。',
  ...
  return [
    '你是一位专业影视编剧。请把用户的创意简述改写为完整的剧本，交给后续分镜师进一步拆分。',
    `风格：${toneDesc}。请写 ${scenes} 个场次。`,
    '语言：默认全部用简体中文写作——场景标题、动作描写、角色名与对白均为中文（内/外景标记也用中文「内景/外景」）；仅当用户创意简述本身明确要求其他语言时才随之。',
    '使用通用剧本格式，每个场次包含：',
    '1) 场号 + 内景/外景 + 地点 + 日/夜',
    '2) 场景动作描写（人物动作、环境氛围，用叙事化散文，不要写镜号/景别/运镜）',
    '3) 角色对白（角色名用中文，对白另起一行）',
...
export const STORYBOARD_SYSTEM_PROMPT = [
  ...
  'title 与 description 一律用简体中文撰写（即使剧本含英文，也翻译成中文镜头描述）。',
