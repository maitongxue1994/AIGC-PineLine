...
/studio           Studio 工作台（核心创作画布，v3「画布优先」）
  · 顶部项目栏（可编辑工程名 · 自动保存 · 分享 · 运行管线 · 导出 · 更多）
  · 左侧「资产」单入口 → 资产抽屉（按类型 tab，汇总画布真实产出）
  · 满屏可平移无限画布 + 节点 + 连线 + 迷你地图 + 画布工具条（撤销/重做/帮助/模板）
  · 节点自治：参数/输出/重命名/运行全在节点上完成（无右侧 Inspector）
  · 空画布模板引导卡（短剧 / 产品广告 / MV）
